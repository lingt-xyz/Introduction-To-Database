<?php 
	$title = "Main Project";
	$sub_title = $title;
	$main_tables = array("Employee", "Department", "Project", "Dependent", "BelongsTo", "ManagedBy", "Dependency", "WorksFor", "LedBy", "Supervise");
?>
<?php 
include 'header.php';
		...
		*/
		...
		while($row = $result->fetch_array()) {
			...
			if(preg_match("#^(Tl|Zr)(.*)$#i", $row[0]) == 0){
				echo "<td><div><h3><a href='./main.php?tableName=" . $row[0] . "'>" . $row[0] . "</a></h4></div></td>";
			}
			*/
			if($td_counter % 5 == 0){
			    echo "<tr>";
			}
			if (in_array($row[0], $main_tables)) {
			    $td_counter++;
			    echo "<td><div><h3><a href='./main.php?tableName=" . $row[0] . "'>" . $row[0] . "</a></h4></div></td>";
			}
			if($td_counter % 5 == 0){
